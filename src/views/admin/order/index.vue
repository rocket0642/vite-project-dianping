<template>
  <div class="order-management">
    ...
    <div class="filter-container">
      <el-input v-model="queryParams.orderNo" placeholder="订单号" style="width: 200px;" class="filter-item"
        @keyup.enter="handleSearch" />
      <el-input v-model="queryParams.userName" placeholder="用户名/手机号" style="width: 200px;" class="filter-item"
        @keyup.enter="handleSearch" />
      ...
      <el-date-picker v-model="queryParams.dateRange" type="daterange" range-separator="至" start-placeholder="开始日期"
        end-placeholder="结束日期" value-format="YYYY-MM-DD" class="filter-item" style="width: 260px" />
    ...
    <el-table v-loading="loading" :data="orderList" border style="width: 100%">
      ...
      <el-table-column label="操作" width="280" align="center">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 1 && row.paymentStatus === 1" type="success" size="small"
            @click="handleShip(row)">
            发货
          </el-button>
          <el-button v-if="row.status === 0 || (row.status === 1 && row.paymentStatus === 0)" type="danger" size="small"
            @click="handleCancel(row)">
            取消
          </el-button>
          <el-button v-if="row.status === 4" type="warning" size="small" @click="handleRefund(row)">
            退款
          </el-button>
        </template>
      </el-table-column>
