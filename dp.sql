...
CREATE TABLE `tb_address`  (
  ...
  `detailed_address` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
...
CREATE TABLE `tb_order`  (
  ...
  `goods_price` bigint(0) UNSIGNED NOT NULL COMMENT '商品价格，单位是分',
  `amount` int(0) UNSIGNED NOT NULL DEFAULT 1 COMMENT '购买数量',
  `total_price` bigint(0) UNSIGNED NOT NULL COMMENT '总价，单位是分',
  `pay_type` tinyint(0) UNSIGNED NOT NULL DEFAULT 1 COMMENT '支付方式 1：余额支付；2：支付宝；3：微信',
  `status` tinyint(0) UNSIGNED NOT NULL DEFAULT 1 COMMENT '订单状态，1：未支付；2：已支付；3：已完成；4：已取消；5：退款中；6：已退款',
